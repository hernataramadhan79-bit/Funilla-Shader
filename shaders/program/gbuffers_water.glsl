...

	{
		...
		if (water > 0.5 || ((translucent + glass) > 0.5 && albedo.a > 0.01 && albedo.a < 0.95)) {
			...
			if (reflection.a < 1.0) {
				...
				cloudAlpha = cloud.a;
				skyReflection = mix(skyReflection, cloud.rgb * 1.5, cloud.a);
				#endif
				#if CLOUDS == 2
			...
			vec3 specular = GetSpecularHighlight(newNormal, viewPos,  0.7, vec3(0.02),
													specularColor, shadow, color.a) * 15.0;
			#if ALPHA_BLEND == 0
			...
			reflection.rgb += specular * (1.0 - reflectionMask) * (1.0 - cloudAlpha) / specularAlpha;
			#endif
			...
			vec3 specular = GetSpecularHighlight(newNormal, viewPos, smoothness, baseReflectance,
										   	   specularColor, shadow * vanillaDiffuse, color.a);
											   
			albedo.rgb += specular * (1.0 - cloudAlpha);
			#endif
		}
