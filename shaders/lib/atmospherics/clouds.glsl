...
float CloudSampleBaseFunilla(vec2 coord) {
	float noiseWorley = texture2D(noisetex, coord * 1.5).g;
	noiseWorley = pow(1.0 - noiseWorley, 2.0) * 0.5 + 0.25;

	float noisePerlin = texture2D(noisetex, coord * 0.75).r;

	float noiseBase = mix(noisePerlin, noiseWorley, 0.65);
	
	// Create hard cutoffs for anime cel-shaded look
	noiseBase = smoothstep(0.35, 0.65, noiseBase) * 0.8 + 0.2 * noiseBase;
...
float CloudCoverageFunilla(float cloudGradient) {
	float noiseCoverage = 0.0;
	
	if (cloudGradient < 0.1) {
		noiseCoverage = (0.1 - cloudGradient) * 20.0; // Flat bottom
	} else {
		noiseCoverage = (cloudGradient - 0.1) * 2.0; // Puffy top
	}
...
float CloudCombineFunilla(float noiseBase, float noiseCoverage) {
	float noise = (noiseBase - noiseCoverage) * 2.5;
...
float CloudSample(vec2 coord, vec2 wind, float cloudGradient, float sunCoverage, float dither) {
	...
	return noise;
	#elif CLOUD_BASE == 3
	vec2 baseCoord = coord * 0.35 + wind * 1.5;

	float noiseBase = CloudSampleBaseFunilla(baseCoord);
	float noiseCoverage = CloudCoverageFunilla(cloudGradient);

	float noise = CloudCombineFunilla(noiseBase, noiseCoverage);
